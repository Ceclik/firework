...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class Vignette : PostProcessEffectSettings
    {
        ...
        public VignetteModeParameter mode = new() { value = VignetteMode.Classic };

        [Tooltip("Vignette color. Use the alpha channel for transparency.")]
        public ColorParameter color = new() { value = new Color(0f, 0f, 0f, 1f) };
        ...
        public FloatParameter smoothness = new() { value = 0.2f };

        [Range(0f, 1f)] [Tooltip("Lower values will make a square-ish vignette.")]
        public FloatParameter roundness = new() { value = 1f };

        [Tooltip("Should the vignette be perfectly round or be dependent on the current aspect ratio?")]
        public BoolParameter rounded = new() { value = false };

        [Tooltip("A black and white mask to use as a vignette.")]
        public TextureParameter mask = new() { value = null };

        [Range(0f, 1f)] [Tooltip("Mask opacity.")]
        public FloatParameter opacity = new() { value = 1f };
