...
public class MoveCamera : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKey(KeyCode.W) || Input.GetKey(KeyCode.UpArrow))
            transform.position += new Vector3(TheCamera.forward.x, 0, TheCamera.forward.z).normalized * Time.deltaTime *
                                  MoveSpeed;
        else if (Input.GetKey(KeyCode.S) || Input.GetKey(KeyCode.DownArrow))
            transform.position -= new Vector3(TheCamera.forward.x, 0, TheCamera.forward.z).normalized * Time.deltaTime *
                                  MoveSpeed;
        if (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.LeftArrow))
            transform.position -= new Vector3(TheCamera.right.x, 0, TheCamera.right.z).normalized * Time.deltaTime *
                                  MoveSpeed;
