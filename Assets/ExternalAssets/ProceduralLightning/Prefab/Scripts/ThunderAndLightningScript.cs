...
namespace DigitalRuby.ThunderAndLightning
{
    public class ThunderAndLightningScript : MonoBehaviour
    {
        ...
        public RangeOfFloats LightningIntervalTimeRange = new() { Minimum = 10.0f, Maximum = 25.0f };
        ...
        [Tooltip("The chance lightning will simply be in the clouds with no visible bolt")] [Range(0.0f, 1.0f)]
        ...
        [Tooltip("Base point light range for lightning bolts. Increases as intensity increases.")] [Range(1, 10000)]
        public float BaseLightRange = 2000.0f;

        [Tooltip("Starting y value for the lightning strikes")] [Range(0, 100000)]
        ...
        public float SkyboxExposureOriginal { get; private set; }
        ...
        private void Start()
        {
            ...
            if (Camera.farClipPlane < 10000.0f && !Camera.orthographic)
                UnityEngine.Debug.LogWarning("Far clip plane should be 10000+ for best lightning effects");
            ...
            SkyboxExposureOriginal = skyboxExposureStorm =
                skyboxMaterial == null || !skyboxMaterial.HasProperty("_Exposure")
                    ? 1.0f
        ...
        private void Update()
        {
            if (lightningBoltHandler != null && EnableLightning) lightningBoltHandler.Update();
        }
        ...
        private class LightningBoltHandler
        {
            ...
            {
                if (script.lightningInProgress) return;
            ...
            public IEnumerator ProcessLightning(Vector3? _start, Vector3? _end, bool intense, bool visible)
            {
                ...
                if (script.skyboxMaterial != null && script.ModifySkyboxExposure)
                    script.skyboxMaterial.SetFloat("_Exposure",
                        Mathf.Max(intensity * 0.5f, script.skyboxExposureStorm));
                ...
                var playThunder = intensity >= 1.0f;
            ...
            private void Strike(Vector3? _start, Vector3? _end, bool intense, float intensity, Camera camera,
                Camera visibleInCamera)
            {
                // find a point around the camera that is not too close
                const float minDistance = 500.0f;
                var minValue = intense ? -1000.0f : -5000.0f;
                var maxValue = intense ? 1000 : 5000.0f;
                var closestValue = intense ? 500.0f : 2500.0f;
                var x = UnityEngine.Random.Range(0, 2) == 0
                    ? UnityEngine.Random.Range(minValue, -closestValue)
                    : UnityEngine.Random.Range(closestValue, maxValue);
                var y = script.LightningYStart;
                var z = UnityEngine.Random.Range(0, 2) == 0
                    ? UnityEngine.Random.Range(minValue, -closestValue)
                    : UnityEngine.Random.Range(closestValue, maxValue);
                var start = script.Camera.transform.position;
                start.x += x;
                start.y = y;
                start.z += z;
                ...
                if (visibleInCamera != null)
                {
                    ...
                    var q = visibleInCamera.transform.rotation;
                    visibleInCamera.transform.rotation = Quaternion.Euler(0.0f, q.eulerAngles.y, 0.0f);
                    var screenX = UnityEngine.Random.Range(visibleInCamera.pixelWidth * 0.1f,
                        visibleInCamera.pixelWidth * 0.9f);
                    var ScreenZ = UnityEngine.Random.Range(visibleInCamera.nearClipPlane + closestValue + closestValue,
                        maxValue);
                    var point = visibleInCamera.ScreenToWorldPoint(new Vector3(screenX, 0.0f, ScreenZ));
                    start = point;
                    start.y = y;
                ...
                RaycastHit hit;
                if (Physics.Raycast(start, (start - end).normalized, out hit, float.MaxValue)) end = hit.point;

                var generations = script.LightningBoltScript.Generations;
                var trunkWidth = script.LightningBoltScript.TrunkWidthRange;
            ...
            private void CheckForLightning()
            {
                ...
                {
                    var intense = UnityEngine.Random.value < script.LightningIntenseProbability;
