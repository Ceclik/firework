...
namespace DigitalRuby.ThunderAndLightning
{
    ...
    {
        [Header("Direction and distance")] [Tooltip("The start point of the spell. Set this to a muzzle end or hand.")]
        ...
        [Tooltip("Collision force mode")] public ForceMode CollisionForceMode = ForceMode.Impulse;
        ...
        [Tooltip("Collision audio source")] public AudioSource CollisionAudioSource;
        ...
        public RangeOfFloats CollisionVolumeRange = new() { Minimum = 0.4f, Maximum = 0.6f };
        ...
        [Header("Emission")] [Tooltip("Emission sound")]
        ...
        /// </summary>
        public bool CanCastSpell => !Casting && CooldownTimer <= 0.0f;
        ...
        {
            if (EmissionLight != null) EmissionLight.enabled = false;
        }
        ...
        protected void ApplyCollisionForce(Vector3 point)
        {
            ...
            {
                var colliders = Physics.OverlapSphere(point, CollisionRadius, CollisionMask);
                foreach (var c in colliders)
                {
                    ...
                    if (r != null)
                    {
                        if (CollisionIsExplosion)
                            r.AddExplosionForce(CollisionForce, point, CollisionRadius, CollisionForce * 0.02f,
                                CollisionForceMode);
                        else
        ...
        protected void PlayCollisionSound(Vector3 pos)
        {
            ...
            {
                var index = Random.Range(0, CollisionAudioClips.Length - 1);
                var volume = Random.Range(CollisionVolumeRange.Minimum, CollisionVolumeRange.Maximum);
        ...
        public bool CastSpell()
        {
            ...
            OnCastSpell();
            if (Duration > 0.0f) StopAfterSeconds(Duration);
            if (EmissionParticleSystem != null) EmissionParticleSystem.Play();
            ...
            if (EmissionSound != null) EmissionSound.Play();
            return true;
        }
        ...
        public void StopSpell()
        {
            ...
            {
                stopToken++;
                if (EmissionParticleSystem != null) EmissionParticleSystem.Stop();
                if (EmissionLight != null) EmissionLight.enabled = false;
                if (EmissionSound != null && EmissionSound.loop) EmissionSound.Stop();
                DurationTimer = 0.0f;
                Casting = false;
                OnStopSpell();
            }
        ...
        public static GameObject FindChildRecursively(Transform t, string name)
        {
            ...
            {
                var obj = FindChildRecursively(t.GetChild(i), name);
