...
[Serializable] class Tile {
	...
	Color color;
	int[] meshIndeces;
	Action<int[], Vector3[]> rotate;
	Action<int[], Color> changeColor;
...
void ChangeMeshPositions(int[] indeces, Vector3[] positions) {
	for (int i = 0; i < indeces.Length; ++i) meshPositions[indeces[i]] = positions[i];
}

void ChangeMeshColors(int[] indeces, Color color) {
	foreach(int index in indeces) meshColors[index] = color;
}

// Too expensive to upload the arrays to the GPU willy-nilly.
public void UpdateMeshPositions() {mesh.vertices = meshPositions;}
public void UpdateMeshColors() {mesh.colors = meshColors;}

void Awake() {
	tiles = new Tile[rows.Length][];
	Vector2 halfTileSize = tileSize * .5F;
	...
	var tileVertIndeces = new List<int>(new int[]{0,1,2,3});
	...
	for (int rowIndex = 0; rowIndex < rows.Length; ++rowIndex) {
		...
		for (int columnIndex = 0; columnIndex < tiles[rowIndex].Length; ++columnIndex) {
			Color tileColor = rows[rowIndex].colors[columnIndex];	
			...
			for (int i = 0; i < 4; ++i) tileVertPositions[i] += tileCenter;
			meshPositions.AddRange(tileVertPositions);
			tileCenter.x += tileSize.x;
			
			int[] tileTriangles = {0,1,2, 0,2,3};
			for (int i = 0; i < 6; ++i) tileTriangles[i] += tileVertIndeces[0];
...
void Update() {
	foreach (Tile[] row in tiles)
		foreach (Tile tile in row) tile.Rotation = Time.time * rotationSpeed;
	UpdateMeshPositions();
	
	if(Input.GetMouseButtonDown(0)) {
		for (int rowIndex = 0; rowIndex < tiles.Length; ++rowIndex) {
			Color leftColor = tiles[rowIndex][0].Color;
			for (int columnIndex = 0; columnIndex < tiles[rowIndex].Length - 1; ++columnIndex)
