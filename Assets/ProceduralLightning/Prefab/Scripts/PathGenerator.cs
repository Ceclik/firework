...
namespace DigitalRuby.ThunderAndLightning
{
    ...
    public static class PathGenerator
    {
        ...
        /// <summary>
        ///     Adjust how curves are calculated. This value should be positive. Generally leave as is unless you want some gnarly
        ///     effects.
        /// </summary>
        private const float curveMultiplier = 3.0f;

        /// <summary>
        ///     Adjusts how the spline curves. This value should be negative. Generally leave as is unless you want some gnarly
        ///     effects.
        /// </summary>
        private const float splineMultiplier1 = -3.0f;

        /// <summary>
        ///     Adjusts how the spline curves. This value should be positive. Generally leave as is unless you want some gnarly
        ///     effects.
        /// </summary>
        ...
        private static void GetCurvePoint2D(ref Vector3 start, ref Vector3 end, ref Vector3 ctr1, ref Vector3 ctr2,
            float t, out Vector3 point)
        {
            var tSquared = t * t;
            var tCubed = tSquared * t;
            var p1x = curveMultiplier * (ctr1.x - start.x);
            var p1y = curveMultiplier * (ctr1.y - start.y);
            var p2x = curveMultiplier * (ctr2.x - ctr1.x) - p1x;
            var p2y = curveMultiplier * (ctr2.y - ctr1.y) - p1y;
            var p3x = end.x - start.x - p1x - p2x;
            var p3y = end.y - start.y - p1y - p2y;
            var finalX = start.x + p1x * t + p2x * tSquared + p3x * tCubed;
            var finalY = start.y + p1y * t + p2y * tSquared + p3y * tCubed;

            point = new Vector3(finalX, finalY, 0.0f);
        }

        private static void GetCurvePoint3D(ref Vector3 start, ref Vector3 end, ref Vector3 ctr1, ref Vector3 ctr2,
            float t, out Vector3 point)
        {
            var tSquared = t * t;
            var tCubed = tSquared * t;
            var p1x = (ctr1.x - start.x) * curveMultiplier;
            var p1y = (ctr1.y - start.y) * curveMultiplier;
            var p1z = (ctr1.z - start.z) * curveMultiplier;
            var p2x = (ctr2.x - ctr1.x) * curveMultiplier - p1x;
            var p2y = (ctr2.y - ctr1.y) * curveMultiplier - p1y;
            var p2z = (ctr2.z - ctr1.z) * curveMultiplier - p1z;
            var p3x = end.x - start.x - p1x - p2x;
            var p3y = end.y - start.y - p1y - p2y;
            var p3z = end.z - start.z - p1z - p2z;
            var finalX = start.x + p1x * t + p2x * tSquared + p3x * tCubed;
            var finalY = start.y + p1y * t + p2y * tSquared + p3y * tCubed;
            var finalZ = start.z + p1z * t + p2z * tSquared + p3z * tCubed;
        ...
        */
        ...
        private static void CalculateNonuniformCatmullRom(float p1, float p2, float p3, float p4, float distance1,
            float distance2, float distance3, out Vector4 point)
        {
            // http://stackoverflow.com/questions/9489736/catmull-rom-curve-with-no-cusps-and-no-self-intersections
            // compute coefficients for a nonuniform Catmull-Rom spline

            var t1 = ((p2 - p1) / distance1 - (p3 - p1) / (distance1 + distance2) + (p3 - p2) / distance2) * distance2;
            var t2 = ((p3 - p2) / distance2 - (p4 - p2) / (distance2 + distance3) + (p4 - p3) / distance3) * distance2;
        ...
        private static void ClampSplineDistances(ref float distance1, ref float distance2, ref float distance3)
        {
            ...
            if (distance2 < splineEpsilon) distance2 = splineDistanceClamp;
            if (distance1 < splineEpsilon) distance1 = distance2;
            if (distance3 < splineEpsilon) distance3 = distance2;
        }

        private static void GetSplinePoint2D(ref Vector3 point1, ref Vector3 point2, ref Vector3 point3,
            ref Vector3 point4, float t, out Vector3 point)
        {
            var distance1 = Distance2D(ref point1, ref point2);
            var distance2 = Distance2D(ref point2, ref point3);
            var distance3 = Distance2D(ref point3, ref point4);

            ClampSplineDistances(ref distance1, ref distance2, ref distance3);

            // plot the spline on the x and y axis
            Vector4 polynomialX;
            CalculateNonuniformCatmullRom(point1.x, point2.x, point3.x, point4.x, distance1, distance2, distance3,
                out polynomialX);
            Vector4 polynomialY;
            CalculateNonuniformCatmullRom(point1.y, point2.y, point3.y, point4.y, distance1, distance2, distance3,
                out polynomialY);

            point = new Vector3(CalculatePolynomial(ref polynomialX, t), CalculatePolynomial(ref polynomialY, t), 0.0f);
        }

        private static void GetSplinePoint3D(ref Vector3 point1, ref Vector3 point2, ref Vector3 point3,
            ref Vector3 point4, float t, out Vector3 point)
        {
            var distance1 = Distance3D(ref point1, ref point2);
            var distance2 = Distance3D(ref point2, ref point3);
            var distance3 = Distance3D(ref point3, ref point4);
            ...
            CalculateNonuniformCatmullRom(point1.y, point2.y, point3.y, point4.y, distance1, distance2, distance3,
                out polynomialY);
            Vector4 polynomialZ;
            CalculateNonuniformCatmullRom(point1.z, point2.z, point3.z, point4.z, distance1, distance2, distance3,
                out polynomialZ);

            point = new Vector3(CalculatePolynomial(ref polynomialX, t), CalculatePolynomial(ref polynomialY, t),
                CalculatePolynomial(ref polynomialZ, t));
        }
        ...
        public static float CreateCurve(ICollection<Vector3> path, Vector3 start, Vector3 end, Vector3 ctr1,
            Vector3 ctr2, int numberOfSegments, float startT)
        {
            ...
            float t;
            var tIncrement = 1.0f / (numberOfSegments + 1);
            Vector3 point;
        ...
        /// <summary>
        ///     Creates a spline path that curves around points in a list. Each source segment will have the same number of spline
        ///     points.
        /// </summary>
        /// <param name="path">Receives the path points. Collection is not cleared.</param>
        /// <param name="points">Points for the spline to follow. Must contain at least 4 points.</param>
        /// <param name="numberOfSegments">
        ///     Total number of line segments for the spline. The higher this number, the more points
        ///     are sampled.
        /// </param>
        ...
        public static bool CreateSpline(ICollection<Vector3> path, IList<Vector3> points, int numberOfSegments,
            bool closePath)
        {
            if (points.Count < MinPointsForSpline) return false;

            // make sure number of segments isn't too large or small
            numberOfSegments = Math.Min(1024, Math.Max(numberOfSegments, 4));
            var numberOfPoints = closePath ? points.Count : points.Count - 1;
            int previousPointIndex, currentPointIndex, nextPointIndex, nextNextPointIndex;
            var closePathInt = closePath ? 1 : 0;
            float t;
            var tIncrement = 1.0f / numberOfSegments * numberOfPoints;
            var start = 0.0f;
            ...
            for (currentPointIndex = 0; currentPointIndex < numberOfPoints; currentPointIndex++)
            {
                ...
                if (closePath && nextPointIndex > numberOfPoints - 1) nextPointIndex -= numberOfPoints;
                ...
                if (Is2D)
                    ...
                    {
                        GetSplinePoint2D(ref previousPoint, ref currentPoint, ref nextPoint, ref nextNextPoint, t,
                            out point);
                        path.Add(point);
                    }
                else
                    ...
                    for (t = start; t <= 1.0f; t += tIncrement)
                    {
                        GetSplinePoint3D(ref previousPoint, ref currentPoint, ref nextPoint, ref nextNextPoint, t,
                            out point);
                        path.Add(point);
                    }
        ...
        /// <summary>
        ///     Creates a spline path that curves around points in a list. This tries to maintain a similar distance between spline
        ///     segments, regardless of distance between the initial points.
        /// </summary>
        /// <param name="path">Receives the path points. Collection is not cleared.</param>
        /// <param name="points">Points for the spline to follow. Must contain at least 4 points.</param>
        ...
        /// <param name="closePath">True to loop back to the start, false otherwise</param>
        /// <returns>True if success, false if points length is too small</returns>
        public static bool CreateSplineWithSegmentDistance(ICollection<Vector3> path, IList<Vector3> points,
            float distancePerSegment, bool closePath)
        {
            if (points.Count < MinPointsForSpline || distancePerSegment <= 0.0f) return false;
