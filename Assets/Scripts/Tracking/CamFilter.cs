...
namespace Tracking
{
    public class CamFilter : MonoBehaviour
    {
        ...
        private byte[] _cropPreview;

        private bool _drawCircles;
        private byte[] _rawImage;
        private byte[] _rawImagePreview;

        private Texture2D _tex;
        private Texture2D _texCrop;

        private Texture2D _texPreview;
        ...
        private void Start()
        {
            ...
            _capture.Start();
            _tex = new Texture2D(_capture.Width, _capture.Height, TextureFormat.RGB24, false, true);
            _texPreview = new Texture2D(_capture.Width, _capture.Height, TextureFormat.RGB24, false, true);
            _texCrop = new Texture2D(_capture.Width, _capture.Height, TextureFormat.RGB24, false, true);

            cropImage.texture = _texCrop;
            filteredImage.texture = _tex;
            previewImage.texture = _texPreview;
        }
        ...
        private void Update()
        {
            if (_rawImage != null)
            {
                _tex.LoadRawTextureData(_rawImage);
                _tex.Apply();
            }

            if (_rawImagePreview != null)
            {
                _texPreview.LoadRawTextureData(_rawImagePreview);
                _texPreview.Apply();
            }

            if (_cropPreview != null)
            {
                _texCrop.LoadRawTextureData(_cropPreview);
                _texCrop.Apply();
            }
        ...
        private void ProcessFrame(object sender, EventArgs arg)
        {
            ...
            if (_capture != null && _capture.Ptr != IntPtr.Zero)
            {
                ...
                if (_frame != null)
                {
                    ...
                    _rawImage = _hsv.GetData(); //Images assignment
                    _rawImagePreview = BgrToRgb(_frame).GetData();
