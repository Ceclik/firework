...
namespace Settings
{
    ...
    public class GameSettings
    {
        ...
        {
            for (var i = 0; i < levelSettings.Length; i++) levelSettings[i] = LevelSettings.Default();
        }

        public void SaveSettings()
        {
            for (var i = 0; i < levelSettings.Length; i++)
            {
                var structType = typeof(LevelSettings);
                ...
                foreach (var field in fields)
                {
                    if (field.FieldType == typeof(double) || field.FieldType == typeof(float))
                    {
                        ...
                        PlayerPrefs.SetFloat(field.Name + "_" + i, value);
                    }

                    if (field.FieldType == typeof(bool))
                        //Debug.Log("Field name:" + field.Name + ", Field value:" + ((bool)field.GetValue(this)).ToString());                
                        PlayerPrefs.SetString(field.Name + "_" + i,
                            ((bool)field.GetValue(levelSettings[i])).ToString());
                }
            }
        }
        ...
        {
            for (var a = 0; a < levelSettings.Length; a++)
            {
                var structType = typeof(LevelSettings);
                var fields = structType.GetFields();
                ...
                {
                    var field = fields[i];
                    if (PlayerPrefs.HasKey(field.Name + "_" + a))
                    {
                        if (field.FieldType == typeof(double) || field.FieldType == typeof(float))
                        {
                            var value = PlayerPrefs.GetFloat(field.Name + "_" + a);
                            //Debug.Log("Field name:" + field.Name + ", Field value:" + value.ToString());
                        ...
                        if (field.FieldType == typeof(bool))
                        {
                            var value = Convert.ToBoolean(PlayerPrefs.GetString(field.Name + "_" + a));
                            //Debug.Log("Field name:" + field.Name + ", Field value:" + value.ToString());
